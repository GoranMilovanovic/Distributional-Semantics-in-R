...
With a mean accuracy of 73% we cannot say that {openNLP} performance falls short. Once again: named entitiry recognition in Shakespeare's plays (again: 16/17th century English) is a serious task for any recognition system. Let's take a look at the results *prima facie*:
...
**N.B.** `playList2` contains information on *The Two Noble Kinsmen*, a play not included in our `corpus`, published in 1634 and attributed to John Fletcher and William Shakespeare (cf. Wikipedia article on [The Two Noble Kinsmen](https://en.wikipedia.org/wiki/The_Two_Noble_Kinsmen)).

Thus, a small intervention:

``` {r echo = T}
playList2$Title <- str_trim(playList2$Title, side = "right") # some trailing whitespace goes
playList2$City <- str_trim(playList2$City, side = "right")
playList2$Country_Modern <- str_trim(playList2$Country_Modern, side = "right")
playList2 <- playList2[-which(playList2$Title == "The Two Noble Kinsmen"), ]
```

Now, the problem here is that the play titles, as given in `playList2`, do not match perfectly with the titles in `corpus` (we have  used the `id` field to store play titles in `corpus`). Did I ever told you about *fuzzy string matching* - something immensely useful in text-mining and related applications? You might wish to do `install.packages("stringdist")` first.
...
# {stringdist}
library(stringdist)
corpusTitles <- unlist(meta(corpus, tag = "id"))
playList2Title <- playList2$Title
distTitles <- stringdistmatrix(corpusTitles, playList2Title, method = c("dl"))
distTitles <- distTitles %>% as.data.frame()
```

The `distTitles` now contains a matrix of *Damerau–Levenshtein distances* between the titles of Shakespeare's plays as found in our `corpus` against the titles as provided in `playList2`. The *Damerau–Levenshtein distance* *"...  is a distance (string metric) between two strings, i.e., finite sequence of symbols, given by counting the minimum number of operations needed to transform one string into the other, where an operation is defined as an insertion, deletion, or substitution of a single character, or a transposition of two adjacent characters."* (source: [Damerau–Levenshtein distance Wikipedia Article](https://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance))

The entries from `corpus` are found in rows; let's see what are the corresponding, minimal distance entries in each column:

``` {r echo = T}
matchTitles <- apply(distTitles, 1, function(x) {return(which.min(x))})
```

Are the results consistent?

``` {r echo = T}
sum((duplicated(matchTitles)))
```

Check:

``` {r echo = T}
matchFrame <- data.frame(corpus = corpusTitles,
                         new = playList2$Title[matchTitles],
                         stringsAsFactors = F)
head(matchFrame)
```
...
meta(corpus, tag = "description2", type = "local") <- 
  playList2$Genre[matchTitles]
meta(corpus, tag = "city", type = "local") <- 
  playList2$City[matchTitles]
meta(corpus, tag = "country", type = "local") <- 
  playList2$Country_Modern[matchTitles]
meta(corpus, tag = "countryNote", type = "local") <- 
  playList2$CountryNote[matchTitles]
### --- writeCorpus()
